...
class Model(nn.Module):
    ...
    def forward(self, qus, ans, qus_len, ans_len):
        ...
        hidden1 = (self.initHidden(batch_size), self.initHidden(batch_size))
        hidden2 = (self.initHidden(batch_size), self.initHidden(batch_size))
        # |hidden| = (2, num_layers*num_directions, batch_size, hidden_size)
        ...
        # tfidf_qus, tfidf_ans = F.softmax(tfidf_qus, 1), F.softmax(tfidf_ans, 1)
        ...
        # feed to lstm
        pack_output_qus, (h_qus, c_qus) = self.lstm_q(pack_qus, hidden1)
        pack_output_ans, (h_ans, c_ans) = self.lstm_a(pack_ans, hidden2)
